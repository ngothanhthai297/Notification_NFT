...
exports.index = async (req, res, next) => {
    ...
    try {
        ...
        Notification_Model.find({ address: req.body.address }, ['address', 'title', 'content', 'images', 'type'])
...
exports.update = async (req, res) => {
    ...
    if (req.body.id && !req.body.address) {
        try {
            const checkExistId = await Notification_Model.find({ _id: req.body.id })
            Notification_Model.findOneAndUpdate({ id: req.body.id }, { $set: { status: req.body.status } }, (err, msg) => {
                if (err) {
                    res.status(500).json({
                        status: false,
    ...
    } else if (req.body.address && !req.body.id) {
        try {
        ...
        res.status(400).json({
            status: false,
            message: "Please Choose Update By Id Or Address",
        })
    }
